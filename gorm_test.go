...
func (s *GormTestSuite) TestDNS() {
	tests := []struct {
	...
	}{
		{
			...
			config: contracts.FullConfig{
				Config: contracts.Config{
					Dsn: "sqlserver://user:pass@localhost:1433?database=testdb&charset=utf8mb4&MultipleActiveResultSets=true",
				},
			},
			expected: "sqlserver://user:pass@localhost:1433?database=testdb&charset=utf8mb4&MultipleActiveResultSets=true",
		},
		{
			name: "without dsn",
			config: contracts.FullConfig{
